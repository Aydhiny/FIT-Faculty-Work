...
public:
    ...
    //2.3.
    const char* getSjedisteId() const { return (_sjedisteId == nullptr) ? "" : _sjedisteId; }
    ...
    */
    void setSjedista(int brojRedova, int brojKolona) {
        for (int i = 0; i < _brojRedova; i++) {
            ...
            delete[] _sjedista[i]; //dealociranje i-tog reda
        }
...
public:
    ...
    bool getSpol() const { return (_spol == nullptr) ? true : *_spol; }
    const char* getRadnoMjesto() const { return _radnoMjesto; }
    const char* getGrad() const { return (_grad == nullptr) ? "" : _grad; }
